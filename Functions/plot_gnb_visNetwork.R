...
plot_gnb_visNetwork <- function(nodes = nodes, 
                            edges = edges){
  library(visNetwork)
  visNetwork(nodes, edges, width = "100%")  %>%
    visInteraction(navigationButtons = TRUE, hover = TRUE) %>% #增加一些控件来对图进行移动啊，放大缩小啊
    ...
    visEvents(hoverNode = "function(nodes) {
        Shiny.onInputChange('gnb_current_node_id', nodes);
      ;}") %>% #悬停到该节点时，设置输入参数current_node_id,即获取当前节点的id号
    visExport()
}